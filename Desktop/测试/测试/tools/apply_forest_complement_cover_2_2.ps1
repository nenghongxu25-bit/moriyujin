...
$backupPath = Join-Path $root "assets\tileset\forest.before_complement_2_2_20260830.png"
$tempPath = Join-Path $root "assets\tileset\forest.complement_2_2.tmp.png"
...
try {
    ...
    for ($turns = 0; $turns -lt 4; $turns++) {
        ...
        $covered += Copy-RotatedMaskedTile $bitmap $sourcePixels $edgeOrigin $turns { param($x, $y) $x -ge 64 }
        ...
        $covered += Copy-RotatedMaskedTile $bitmap $sourcePixels $cornerOrigin $turns { param($x, $y) ($x + $y) -ge 192 }
...
[PSCustomObject]@{
    ...
    TargetSubAtlas = "(2,2)"
    ComplementTargets = $targets -join "; "
