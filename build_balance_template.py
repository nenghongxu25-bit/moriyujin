...
def sheet_xml(data):
    ...
    for row_idx, row in enumerate(data, 1):
        rows_xml.append(
            f'<row r="{row_idx}">'
            + "".join(cell_xml(value, row_idx, col_idx) for col_idx, value in enumerate(row))
            + "</row>"
        )
    return (
...
def sumifs(col: str, role_cell: str, attr: str):
    return (
        f"SUMIFS(RoleParams!${col}:${col},RoleParams!$A:$A,${role_cell},"
        f"RoleParams!$D:$D,{quoted(attr)})"
    )


def equip_sum(plan_cell: str, attr: str):
    return (
        f"SUMIFS(EquipPlans!$I:$I,EquipPlans!$A:$A,${plan_cell},"
        f"EquipPlans!$B:$B,{quoted(attr)})"
    )


def role_value(row: int, attr: str):
    role_cell = f"B{row}"
    return (
        f'IF($A{row}="","",'
        f'{sumifs("E", role_cell, attr)}+'
        f'{sumifs("F", role_cell, attr)}*'
        f'{sumifs("B", role_cell, attr)}*'
        f'{sumifs("G", role_cell, attr)}+'
        f'{sumifs("H", role_cell, attr)})'
    )
...
def main():
    ...
    out = Path("role_balance_template.xlsx")
    if out.exists():
        out.unlink()
    ...
    role_rows = [["\u89d2\u8272", "\u7b49\u7ea7", "\u5c5e\u6027\u5206\u7c7b", "\u5c5e\u6027\u540d", "\u57fa\u7840\u6570\u503c", "\u6210\u957f/\u7ea7", "\u8d44\u8d28", "\u7ecf\u8109"]]
    category = ""
    for row in rows[2:]:
    ...
    for source_row in role_rows[1:]:
        role_rows.append(["\u65b0\u89d2\u8272"] + source_row[1:])
    ...
    for row in rows[2:19]:
        ...
        if any(value != "" for value in equip):
            ...
            equip_rows.append(
                ["\u6807\u51c6\u6d4b\u8bd5\u88c5", attr]
                + equip
                + [("formula", f"SUM(C{excel_row}:H{excel_row})")]
            )

    test_rows = [
        ["\u6d4b\u8bd5ID", "\u89d2\u8272", "\u88c5\u5907\u65b9\u6848", "\u5907\u6ce8"],
        ["T001", role, "\u6807\u51c6\u6d4b\u8bd5\u88c5", "\u57fa\u51c6"],
        ["T002", "\u65b0\u89d2\u8272", "\u6807\u51c6\u6d4b\u8bd5\u88c5", "\u590d\u5236\u9752\u69d0\u53c2\u6570\u540e\u8c03\u6574"],
    ]
    ...
    for row in range(2, 22):
        ...
        formulas += [role_value(row, attr) for attr in base_attrs]
        formulas += [
    ...
    print(out.resolve())
    print("role_rows", len(role_rows) - 1, "equip_rows", len(equip_rows) - 1, "test_rows", len(test_rows) - 1)
